...
//Modified SimulationRunDialog
public class SimulationDispersionRunDialog extends JDialog {
	private static final Logger log = LoggerFactory.getLogger(SimulationDispersionRunDialog.class);
	...
	private final double[] simulationX; //Dispersion Results
	private final double[] simulationY; //Dispersion Results
	private final boolean[] simulationDone;
	
	private PrintWriter pw;
	
	public SimulationDispersionRunDialog(Window window, OpenRocketDocument document,DispersionOptions dispopt, Simulation... simulations) {
		//// Running simulations...
		...
		try {
		    pw = new PrintWriter(new File("DispersionData.csv"));
		...
		this.simulations = new Simulation[dispopt.getNumberSims()];
		this.simulations[0]=simulations[0];
		
		Random gen = new Random();
		
		//Populate simulations array
		for(int i=1;i<dispopt.getNumberSims();i++) {
			Simulation sim = simulations[0].copy();
			sim.getOptions().setLaunchRodAngle(dispopt.getRodAngle()+gen.nextGaussian()*dispopt.getRodAngleDeviation());
			//sim.getOptions().setLaunchRodDirection(dispopt.getLaunchDirection()+gen.nextGaussian()*dispopt.getLaunchDirectionDeviation());
			sim.getOptions().setLaunchIntoWind(false);
			sim.getOptions().setLaunchRodDirection(2*Math.PI*gen.nextDouble());
			/*sim.getOptions().setLaunchTemperature(dispopt.getTemperature()+gen.nextGaussian()*dispopt.getTemperatureDeviation());
			sim.getOptions().setLaunchPressure(dispopt.getPressure()+gen.nextGaussian()*dispopt.getPressureDeviation());*/
			sim.getOptions().setWindSpeedAverage(dispopt.getWindSpeed()+gen.nextGaussian()*dispopt.getWindSpeedDeviation());
			/*
			Iterator<MotorMount> iterator = sim.getConfiguration().motorIterator();
			String flightConfigId = sim.getConfiguration().getFlightConfigurationID();
			while (iterator.hasNext()) {
				MotorMount mount = iterator.next();
				MotorConfiguration motorConfig = mount.getMotorConfiguration().get(flightConfigId);
				motorConfig.setEjectionDelay(motorConfig.getEjectionDelay()*(1+dispopt.getDragDeviation()));
			}// This does not work*/
			this.simulations[i]=sim;
		}
		...
		simulationX = new double[n];
		simulationY = new double[n];
		simulationDone = new boolean[n];
		
		for (int i = 0; i < n; i++) {
			simulationNames[i] = Integer.toString(i)+"/"+Integer.toString(n);
			simulationWorkers[i] = new InteractiveSimulationWorker(document, this.simulations[i], i);
	 ...
	 */
	public static void runSimulations(Window parent, OpenRocketDocument document, DispersionOptions dispopt, Simulation... simulations) {
		new SimulationDispersionRunDialog(parent, document,dispopt,simulations).setVisible(true);
	}
	...
	private void updateProgress() {
		...
		if (index >= simulations.length) {
			...
			JOptionPane.showMessageDialog(null, simulations.length+" Simulation results saved to DispersionData.csv");
			pw.close();
			this.dispose();
			return;
		}
	...
	private class InteractiveSimulationWorker extends SimulationWorker {
		...
		protected void simulationDone() {
			simulationDone[index] = true;
				...
				//Log dispersion data
				simulationX[index]=simulation.getSimulatedData().getBranch(0).getLast(FlightDataType.TYPE_POSITION_X);
				simulationY[index]=simulation.getSimulatedData().getBranch(0).getLast(FlightDataType.TYPE_POSITION_Y);
				StringBuilder sb = new StringBuilder();
				sb.append(simulationX[index] + ", "+simulationY[index]+"\n");
				pw.print(sb.toString());
				pw.flush();
				if(index%25==0&&index!=0) {//Ask nicely to throw away data
					for(int i=0;i<25;i++) {
						simulations[index-i] = new Simulation(simulations[0].getRocket());
					}
				System.gc();
				}
		...
		protected void simulationInterrupted(Throwable t) {
			...
			if (t instanceof SimulationLaunchException) {
				...
				DetailDialog.showDetailedMessageDialog(SimulationDispersionRunDialog.this,
						new Object[] {
								//// Unable to simulate:
