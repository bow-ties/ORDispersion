...
public class SimulationEditDialog extends JDialog {
	...
	private final DispersionOptions dispopt;
	...
	public SimulationEditDialog(Window parent, final OpenRocketDocument document, Simulation... sims) {
		...
		this.dispopt = new DispersionOptions();
	...
	private void buildEditCard() {
		...
		//// Dispersion options
		tabbedPane.addTab("Dispersion", new SimulationDispersionPanel(simulation[0],dispopt));
		...
		////Dispersion Button
		button = new JButton("Disperse");
		if (!isSingleEdit()) {
			button.setText("Disperse More");
		}
		button.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				copyChangesToAllSims();
				SimulationDispersionRunDialog.runSimulations(parentWindow, SimulationEditDialog.this.document, dispopt, simulation);
				refreshView();
				setVisible(false);
			}
		});
